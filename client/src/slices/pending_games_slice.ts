...
const ongoingGamesSlice = createSlice({
  ...
  reducers: {
    ...
    // Reducer to remove a game by ID
    remove(state, action) {
      const { id } = action.payload; // The ID of the game to remove
      const index = state.gameList.findIndex((g) => g.id === id);
      if (index > -1) {
        state.gameList.splice(index, 1);
        console.log(`[Pending Games Slice] Removed game with id ${id}`);
      }
    },
  },
});
...
export const { upsert, remove } = ongoingGamesSlice.actions;
